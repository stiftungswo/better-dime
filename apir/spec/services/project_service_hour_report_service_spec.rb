...
RSpec.describe ProjectServiceHourReportService do
  ...
  it "labels multiple categories with their percentage but does not split the hours between them" do
    ...
    # This is the behaviour reported as a problem: the hours are the full project
    # total for every category row, not the total multiplied by the category's share.
    expect(report.rows).to eq([[project.id, "Project X", "#{cat_a.id}, #{cat_b.id}", "Cat A (30%), Cat B (70%)", 10.0, 20.0]])
  end

  it "totals the hours per service across all projects in the footer" do
    expect(report.footer).to eq(["", "", "", "Total", 10.0, 20.0])
  end

  it "labels categories as 0% instead of blowing up when all weights are zero" do
    cat_a = create(:project_category, name: "Cat A")
    cat_b = create(:project_category, name: "Cat B")
    ...
    expect(report.rows).to eq([[project.id, "Project X", "#{cat_a.id}, #{cat_b.id}", "Cat A (0%), Cat B (0%)", 10.0, 20.0]])
  end
end
