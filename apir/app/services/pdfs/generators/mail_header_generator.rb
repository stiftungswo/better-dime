  ...
  module Generators
    class MailHeaderGenerator
      ...
      def draw_misc(invoice, project, offer, accountant, costgroups, title_symbol, name)
        ...
        @document.draw_text I18n.t(:invoice) + " Nr.", @default_text_settings.merge(at: [0, @document.cursor])
        @document.draw_text I18n.t(:offer) + " Nr.", @default_text_settings.merge(at: [75, @document.cursor])
        @document.draw_text I18n.t(:project) + " Nr.", @default_text_settings.merge(at: [150, @document.cursor])
        @document.draw_text I18n.t(:date_name), @default_text_settings.merge(at: [330, @document.cursor])
        @document.draw_text @date.strftime("%d.%m.%Y"), @default_text_settings.merge(at: [405, @document.cursor])

        @document.move_down 16

        @document.draw_text invoice.id.to_s, @default_text_settings.merge(at: [0, @document.cursor]) if invoice
        @document.draw_text offer.id.to_s, @default_text_settings.merge(at: [75, @document.cursor]) if offer
        @document.draw_text project.id.to_s, @default_text_settings.merge(at: [150, @document.cursor]) if project
        @document.draw_text I18n.t(:clerk), @default_text_settings.merge(at: [330, @document.cursor])
        @document.draw_text accountant.full_name, @default_text_settings.merge(at: [405, @document.cursor])
      ...
      def draw_logo
        ...
        @document.repeat(lambda { |p| p > 1}) do
          @document.bounding_box([@document.bounds.width - @logo_simple_width, @document.bounds.height + @page_offset], width: @logo_width, height: @logo_height) do
            # @document.stroke_bounds
  
            image_path = Rails.root.join("app", "assets", "logo", "logo_simple.png")
  
            @document.move_up 15
            @document.image image_path, width: @logo_simple_width
          end
        end
      end
