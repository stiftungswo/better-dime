...
module V2
  class ProjectEffortsController < ApplicationController
    ...
    def index
      @filtered = ProjectEffortFilter.new(params).filter ProjectEffort.left_joins(:employee, :costgroup, project_position: [:service, :project, :rate_unit, :position_group])
    ...
    def update_params
      params.permit(:date, :value, :employee_id, :position_id, :costgroup_number)
    end
  end
end
